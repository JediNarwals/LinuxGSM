...
DIR_MOD="$2"
	...
	if [ "$OK" == "0" ]; then
		rm -r ~/WASTELAND/serverfiles/$2/
		cp -aru $DIR_MOD/steamapps/workshop/content/107410/$MOD/. ~/WASTELAND/serverfiles/$2/
		convmv --lower -r --replace --notest ~/WASTELAND/serverfiles/$2/
		cp -au ~/WASTELAND/serverfiles/$2/keys/. ~/WASTELAND/serverfiles/keys/
		cp -au ~/WASTELAND/serverfiles/$2/key/. ~/WASTELAND/serverfiles/keys/
		ShouldRun=1
    exitcode=$?
    if [ ${exitcode} -ne 0 ]; then
